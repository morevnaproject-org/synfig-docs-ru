...
Результирующее значение — `Base ^ Power`, если операция определена.

В неопределённых случаях также возвращается значение (во избежание ошибок):

`0 ^ 0 = 1`\
`0 ^ -x = ±бесконечность`

Если отрицательное основание возведено в нецелую степень, показатель степени округляется (типизируется) до целого числа.
