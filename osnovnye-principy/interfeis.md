...
{% hint style="info" %}
Если интерфейс программы выглядит иначе, возможно, предыдущий пользователь изменил настройки. Чтобы восстановить исходный вид интерфейса, выполните следующие действия:

Перейдите в меню **Окно** → **Рабочая область** → **По умолчанию**.

Рекомендуется также выполнить полный сброс настроек программы:

* В меню выберите **Правка** → **Настройки**.
* В открывшемся окне настроек в нижней части нажмите кнопку **Сбросить настройки**.
{% endhint %}
...
После выбора инструмента пользователь имеет возможность изменить значения параметров инструмента с помощью **Панели настроек инструмента** (2)**.** Содержимое этой панели меняется в зависимости от того, какой у вас инструмент выбран в данный момент.
...
{% hint style="info" %}
Подробную информацию об этих функциях смотрите на странице **Параметр слоя** в разделе **Видимость контрольных точек на рабочей области**.
{% endhint %}
...
* В режиме отображения **Preview** изображения на рабочей области отрисовываются с некоторыми упрощениями, которые незначительно влияют на качество картинки. Например, для масштабирования всех изображений используется линейный тип интерполяции.&#x20;
