...
* **Широкая диафрагма:** Применение широкой диафрагмы (`f/4`) приводит к тому, что объекты вне зоны фокуса размываются сильнее, чем при использовании закрытой диафрагмы (`f/22`).
...
{% hint style="info" %}
Для демонстрации эффекта размытия был анимирован параметр «Размер».\
Его значение изменялось от `0px, 0px` до `200px, 200px` в диапазоне с 0-го по 120-й кадр.
